...
import { EditMemberInfo } from '../Edit/members.edit';
...
const Team = () => {
  ...
  const handleEditModal = (memberID) => {
  ...
  }, [lng]); // Re-fetch when the language changes

  // Re-fetch members when create or edit modal closes
  useEffect(() => {
    if (!create && !edit) {
      fetchMembers();
    }
  }, [create, edit]); // Re-fetch when the modals close
  ...
  return (
    <div className='rounded-[30px] bg-white mdl:rounded-[40px] 3xl:rounded-[100px] mdl:mx-[20px] 3xl:mx-[30px] flex flex-col py-[30px] px-[16px] mdl:py-[50px] mdl:px-[40px] 3xl:p-[70px] mt-[20px] mdl:mt-[25px] 3xl:mt-[30px]'>
      ...
      <div className='flex flex-row flex-wrap justify-between mt-[30px] mdl:mt-[50px]'>
        {members?.map(member => (
          ...
          >
            ...
            <div className='flex flex-row items-center mt-[20px] justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-300'>
              <button onClick={() => handleEditModal(member.id)} className='w-[45%] text-blue-600 border border-dashed py-[15px] px-[20px] flex flex-row items-center text-[16px] gap-[5px] justify-center border-blue-600'>
        ...
        <EditMemberInfo visible={edit} isCloseCreateModal={() => setEditModal(false)} memberID={selectedMember} />
