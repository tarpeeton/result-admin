...
export const CreateMember = ({ isCloseCreateModal, visible }) => {
  ...
  // Handle file upload
  const handleFileUpload = ({ fileList }) => { setFileList(fileList); setMember({ ...member, photo: fileList[0]?.originFileObj || null }); };
  // Remove file handler
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit}>
        ...
        {/* Main Toggle Field */}
        <div className="mt-4">
          <label className='text-[18px] text-titleDark font-bold'>Активен?</label>
          <Switch
